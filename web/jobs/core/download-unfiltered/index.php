...
$prstmt->execute();
$prdata = str_replace('`', '', json_encode($prstmt->fetchAll(PDO::FETCH_ASSOC)));
...
if (!file_exists(__DIR__ . '/restore_point.txt')) {
    ...
    $prdataset = json_decode($prdata, true);
    $prarray = [];
    foreach ($prdataset as $item) {
        $prtags = substr(sanitizeLine(implode(', ', array_slice(explode(', ', explode(PHP_EOL, explode('Tags: ', $item['metadata'])[1] ?? '')[0]) ??
        [], 0, 60))), 0, 500);
        $item['metadata'] = is_null($item['metadata']) ? '' : (strlen($prtags) < 1 ? 'Using MetaData Store...' : $prtags);
        if (!array_key_exists($item['hash'], $prarray)) {
            $filenameParts = explode('-----', $item['filename']);
            $prarray[$item['hash']] = [
              'albums' => [sanitizeLine($item['album'])],
              'filename' => sanitizeLine(substr(end($filenameParts), 0, 20)) . '.' . pathinfo($item['filename'], PATHINFO_EXTENSION),
            ...
            $prarray[$item['hash']]['albums'][] = sanitizeLine($item['album']);
            $prarray[$item['hash']]['times'][] = $item['time'];
        }
    }
    ...
    $favorites = str_replace('$', 'USD', str_replace("'", '-', str_replace('`', '-', json_encode(
      array_values(array_filter(array_merge(array_values($array), array_values($prarray)), function ($item) {
            return in_array('Favorites', $item['albums']);
        })),
        JSON_THROW_ON_ERROR
    ))));
