...
foreach ($deovrSites as $url => $config) {
    ...
    if ($search = $config["search"]) {
        $json = json_decode(
            getUrlWithAuth($url, $config["login"], $config["password"]),
            true
        );
        if (!array_key_exists("scenes", $json) || !is_array($json["scenes"])) {
            echo PHP_EOL .
                "Unsupported DeoVR url that cannot be cached: " .
                $url;
            continue;
        }
        $LibaryKey = array_search(
            "Library",
            array_column($json["scenes"], "name")
        );
        $output = [];
        $changed = false;
        foreach ($search as $s) {
        ...
        foreach ($json["scenes"][$LibaryKey]["list"] as $video) {
            ...
            $tags = array_column(
                array_column($vjson["categories"], "tag"),
                "name"
            );
            ...
            foreach ($search as $s) {
                if (in_array(strtolower($s), $tags)) {
                    $vjson["authorized"] = 1;
                    $encodingKey = array_search(
                        "h265",
                        array_column($vjson["encodings"], "name")
                    );

                    $vs = end(
                        $vjson["encodings"][$encodingKey]["videoSources"]
                    );
                    $videoUrl = $vs["url"];
                    ...
                    $preserve =
                        mediaFile::sanitize(
                            $privuma->getEnv("DEOVR_DATA_DIRECTORY") .
                                DIRECTORY_SEPARATOR .
                                "deovr" .
                                DIRECTORY_SEPARATOR .
                                str_replace(".mp4", "", $filename)
                        ) . ".mp4";
                    if (!$privuma->getCloudFS()->is_file($preserve)) {
                        echo PHP_EOL .
                            "Queueing deovr download to: " .
                            $preserve;
                        $privuma->getQueueManager()->enqueue(
                            json_encode([
                                "type" => "processMedia",
                                "data" => [
                                    "download" => $privuma->getEnv(
                                        "DEOVR_MIRROR"
                                    ),
                                    "preserve" => $preserve,
                                    "skipThumbnail" => true,
