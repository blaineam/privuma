...
$select_results = $conn->query("SELECT id, album, filename FROM media where (filename like '%.mp4' or filename like '%.webm') and url is null and album != 'Favorites' {$album} order by id desc");
...
foreach (array_chunk($results, 2000) as $key => $chunk) {
    foreach ($chunk as $key => $row) {
        ...
        if (!is_null($album) && !is_null($filename)) {
            ...
            $videoPath = $mediaFile->realPath();
            $thumbnailPath = str_replace('.webm', '.jpg', str_replace('.mp4', '.jpg', $videoPath));
