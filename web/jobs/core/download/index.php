...
$opsPlain = new cloudFS($downloadLocationUnencrypted, false, '/usr/bin/rclone', null, false);
...
$stmt = $conn->prepare("SELECT CASE WHEN length(filename) >= 10 THEN substr(filename,-10) ELSE filename END filename, album, dupe, time, hash, REGEXP_REPLACE(metadata, 'www\.[a-zA-Z0-9\_\.\/\:\-\?\=\&]*|(http|https|ftp):\/\/[a-zA-Z0-9\_\.\/\:\-\?\=\&]*', 'Link Removed') as metadata FROM (SELECT * FROM media WHERE (album = 'Favorites' or blocked = 0) and hash is not null and hash != '' and hash != 'compressed') t1 ORDER BY time desc;");
$stmt->execute();
        ...
        <script>
            ...
            function runApp() {
                ...
                function getFolderContent(results, desiredPath, search = "") {
                    if (search.length > 3) {
                        let foundAlbums = findSearch(search).map(
                            (item) => item.album,
                        );
                        return results
                ...
                (async () => {
                    function showAlbum(album, kind) {
                        ...
                        let foundHashes = findSearch(search).map(
                            (item) => item.hash,
                        );
                        let testRes = res.filter((item) =>
                            foundHashes.includes(item.hash),
                        );
                        if (testRes.length > 0) {
                            res = testRes;
                        }
