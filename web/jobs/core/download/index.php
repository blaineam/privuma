                            ...
                            return fetchReader
                              ...
                              .then(async function processRead({
                                ...
                                if (done === true) {
                                  ...
                                  let mergedArray = concatUint8Arrays(...blobs);
                                  var mime =
      ...
      function init() {
        medcrypt.getSrc(
          window.mobileCheck()
            ...
            : (usingRapiServe ? "" : "../") + "ZW/ZW5jcnlwdGVkX2RhdGE=.js",
          (encrypted_data_file) => {
                    ...
                    ([
                      ...
                      isVideo
                        ? jQuery("#content").append(
                            `<a class="gallerypicture" title="${filename}" data-type="${datatype}" data-fancybox="gallery" href="${videoResource}"><img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" class="lazy" data-src="${uri}" loading="lazy" alt=""><script type="text/json">${meta}<\/script>` +
                              (datatype === "video"
                      ...
                      <g>
                        ...
                        : jQuery("#content").append(
                            `<a class="gallerypicture" data-width="1920" href="${uri}" title="${filename}" data-fancybox="gallery"><img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" class="lazy" data-src="${uri}" loading="lazy" alt="" onError="imgError(this)" onLoad="imgLoad(this)"><script type="text/json">${meta}<\/script></a>`
                          )
                  );
              })
              .finally(() => {
...
$mobiledata = MediaCrypto::encryptString($privuma->getEnv('DOWNLOAD_PASSWORD'), "const encrypted_data = `".$mobiledata."`;", false);
$ops->file_put_contents("encrypted_mobile_data.js",$mobiledata);
unset($mobiledata);

$data = MediaCrypto::encryptString($privuma->getEnv('DOWNLOAD_PASSWORD'), "const encrypted_data = ".$data.";", false);
...
if((!is_file($cachePath) || !$cacheStillRecent) && !is_file($rawCachePath)) {
  exec("nice rclone --config ../../../config/rclone/rclone.conf lsjson -R {$downloadLocation} --exclude=\"@eaDir/**\" > {$rawCachePath}");
}
