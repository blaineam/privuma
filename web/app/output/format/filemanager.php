...
//Application Title
define('APP_TITLE', 'Tiny Cloud File Manager');
...
// rclone remote desination with trailing :
$rclone_destination = 'remote:';
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        $write_results = $ops->file_put_contents($file_path, $writedata);
    ...
    if(isset($_POST['type']) && $_POST['type']=="search") {
        $dir = FM_ROOT_PATH;
        $response = scan($dir . '/' . fm_clean_path($_POST['path']), $_POST['content']);
    ...
    if (isset($_POST['type']) && $_POST['type'] == "backup" && !empty($_POST['file'])) {
        ...
        if (!empty($_POST['path'])) {
            $relativeDirPath = fm_clean_path(base64_decode($_POST['path']));
    ...
    if(isset($_POST['type']) && $_POST['type'] == "upload" && !empty($_REQUEST["uploadurl"])) {
        ...
        if ($success) {
            $success = $ops->rename($temp_file, get_file_path(), false);
        }
...
if (isset($_GET['del']) && !FM_READONLY) {
    $del = str_replace( '/', '', fm_clean_path( base64_decode($_GET['del']) ) );
    if ($del != '' && $del != '..' && $del != '.') {
        $path = FM_ROOT_PATH;
        if (FM_PATH != '') {
            $path .= '/' . FM_PATH;
        }
        $is_dir = $ops->is_dir($path . '/' . $del);
...
if (isset($_GET['new']) && isset($_GET['type']) && !FM_READONLY) {
    $type = $_GET['type'];
    $new = str_replace( '/', '', fm_clean_path( strip_tags( base64_decode($_GET['new']) ) ) );
...
if (isset($_GET['ren'], $_GET['to']) && !FM_READONLY) {
    // old name
    $old = base64_decode($_GET['ren']);
    $old = fm_clean_path($old);
    $old = str_replace('/', '', $old);
    // new name
    $new = base64_decode($_GET['to']);
...
if (isset($_GET['st'])) {
    ...
    if ($st != '' && $ops->is_file($path . '/' . $st)) {
        ...
        header('Content-Type: ' . $ops->mime_content_type($path . '/' . $st));
        header('Content-Length:' . $ops->filesize($path . '/' . $st));
        $ops->readfile($path . '/' . $st);
        exit;
    } else {
        fm_set_msg(lng('File not found'), 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(base64_encode(FM_PATH)));
...
if (!empty($_FILES) && !FM_READONLY) {
    ...
    $filename = base64_encode($f['file']['name']);
    $tmp_name = $f['file']['tmp_name'];
    $ext = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
    $isFileAllowed = ($allowed) ? in_array($ext, $allowed) : true;

    if(!fm_isvalid_filename($filename) && !fm_isvalid_filename(base64_decode($_REQUEST['fullpath']))) {
        $response = array (
            'status'    => 'error',
...
if (isset($_POST['chmod']) && !FM_READONLY && !FM_IS_WIN) {
    ...
    $file = fm_clean_path($file);
    $file = str_replace('/', '', $file);
    if ($file == '' || (!$ops->is_file($path . '/' . $file) && !$ops->is_dir($path . '/' . $file))) {
        fm_set_msg(lng('File not found'), 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(base64_encode(FM_PATH)));
    }
...
if (is_array($objects) && fm_is_exclude_items($current_path)) {
    foreach ($objects as $file) {
        ...
        $new_path = $path . '/' . $file;
        if (!$object['IsDir'] && fm_is_exclude_items($file)) {
            $files[] = $file;
        } elseif ($object['IsDir'] && $file != '.' && $file != '..' && fm_is_exclude_items($file)) {
            $folders[] = $file;
        }
    }
}
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <div class="card mb-2 fm-upload-wrapper <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <p class="card-text">
                    <a href="?p=<?php echo base64_encode(FM_PATH) ?>" class="float-right"><i class="fa fa-chevron-circle-left go-back"></i> <?php echo lng('Back')?></a>
                    <?php echo lng('DestinationFolder') ?>: <?php echo fm_enc(fm_convert_win(FM_PATH)) ?>
                </p>

                <form action="<?php echo htmlspecialchars(FM_SELF_URL) . '?p=' . fm_enc(base64_encode(FM_PATH)) ?>" class="dropzone card-tabs-container" id="fileUploader" enctype="multipart/form-data">
                    <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
                    <input type="hidden" name="fullpath" id="fullpath" value="<?php echo fm_enc(base64_encode(FM_PATH)) ?>">
                    <div class="fallback">
        ...
        Dropzone.options.fileUploader = {
            ...
            init: function () {
                this.on("sending", function (file, xhr, formData) {
                    ...
                    document.getElementById("fullpath").value = btoa(_path);
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <p>
                        ...
                        <b><a href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>" class="btn btn-outline-primary"><i class="fa fa-times-circle"></i> <?php echo lng('Cancel') ?></a></b>
                    </p>
                </form>
            </div>
        </div>
...
if (isset($_GET['copy']) && !isset($_GET['finish']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <p>
            <b><a href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;copy=<?php echo urlencode(base64_encode($copy)) ?>&amp;finish=1"><i class="fa fa-check-circle"></i> Copy</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;copy=<?php echo urlencode(base64_encode($copy)) ?>&amp;finish=1&amp;move=1"><i class="fa fa-check-circle"></i> Move</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>"><i class="fa fa-times-circle"></i> Cancel</a></b>
        </p>
        ...
        <ul class="folders break-word">
            ...
            if ($parent !== false) {
                ?>
                <li><a href="?p=<?php echo urlencode(base64_encode($parent)) ?>&amp;copy=<?php echo urlencode(base64_encode($copy)) ?>"><i class="fa fa-chevron-circle-left"></i> ..</a></li>
                <?php
            }
            foreach ($folders as $f) {
                ?>
                <li>
                    <a href="?p=<?php echo urlencode(base64_encode(trim(FM_PATH . '/' . $f, '/'))) ?>&amp;copy=<?php echo urlencode(base64_encode($copy)) ?>"><i class="fa fa-folder-o"></i> <?php echo fm_convert_win($f) ?></a></li>
                <?php
            }
            ?>
        </ul>
    ...
    <div class="col-md-8 offset-md-2 pt-3">
        <div class="card mb-2 <?php echo fm_get_theme(); ?>">
            <h6 class="card-header">
                ...
                <a href="?p=<?php echo base64_encode(FM_PATH) ?>" class="float-right"><i class="fa fa-window-close"></i> <?php echo lng('Cancel')?></a>
            </h6>
            <div class="card-body">
...
// file viewer
if (isset($_GET['view'])) {
    $file = base64_decode($_GET['view']);
    $quickView = (isset($_GET['quickView']) && $_GET['quickView'] == 1) ? true : false;
    $file = fm_clean_path($file, false);
    $file = str_replace('/', '', $file);
    if ($file == '' || !$ops->is_file($path . '/' . $file) || in_array($file, $GLOBALS['exclude_items'])) {
        fm_set_msg(lng('File not found'), 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(base64_encode(FM_PATH)));
    }
    ...
    $file_url = FM_SELF_URL . "/?p=" . urlencode(base64_encode(FM_PATH)) . "&st=" . urlencode(base64_encode($file));
    ...
    if($online_viewer && $online_viewer !== 'false' && in_array($ext, fm_get_onlineViewer_exts())){
        ...
        $is_text = true;
        $content = $ops->file_get_contents($file_path);
    }
        ...
        <div class="col-12">
            <?php if(!$quickView) { ?>
                ...
                <p class="break-word">
                    ...
                    // Image info
                    if ($is_image) {
                        $image_size = getimagesize($ops->pull($file_path));
                ...
                <p>
                    <b><a href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;dl=<?php echo urlencode(base64_encode($file)) ?>"><i class="fa fa-cloud-download"></i> <?php echo lng('Download') ?></a></b> &nbsp;
                    <b><a href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;st=<?php echo urlencode(base64_encode($file)) ?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php echo lng('Open') ?></a></b>
                    &nbsp;
                    <?php
                    if ($is_text && !FM_READONLY) {
                        ?>
                        <b><a href="?p=<?php echo urlencode(base64_encode(trim(FM_PATH))) ?>&amp;edit=<?php echo urlencode(base64_encode($file)) ?>" class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo lng('Edit') ?>
                            </a></b> &nbsp;
                        <b><a href="?p=<?php echo urlencode(base64_encode(trim(FM_PATH))) ?>&amp;edit=<?php echo urlencode(base64_encode($file)) ?>&env=ace"
                              class="edit-file"><i class="fa fa-pencil-square-o"></i> <?php echo lng('AdvancedEditor') ?>
                            </a></b> &nbsp;
                    <?php } ?>
                    <b><a href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>"><i class="fa fa-chevron-circle-left go-back"></i> <?php echo lng('Back') ?></a></b>
                </p>
                <?php
            }
            if($is_onlineViewer) {
...
if (isset($_GET['edit'])) {
    ...
    $file_url = FM_SELF_URL . "/?p=" . urlencode(base64_encode(FM_PATH)) . "&view=" . urlencode(base64_encode($file));
    $file_path = $path . '/' . $file;
    ...
    if (isset($_POST['savedata'])) {
        ...
        $ops->file_put_contents($file_path, $writedata);
    ...
    <div class="path">
        <div class="row">
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-right pt-1">
                <a title="<?php echo lng('Back') ?>" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(base64_encode(trim(FM_PATH))) ?>&amp;view=<?php echo urlencode(base64_encode($file)) ?>"><i class="fa fa-reply-all"></i> <?php echo lng('Back') ?></a>
                <a title="<?php echo lng('BackUp') ?>" class="btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="backup('<?php echo urlencode(trim(FM_PATH)) ?>','<?php echo urlencode($file) ?>')"><i class="fa fa-database"></i> <?php echo lng('BackUp') ?></a>
                <?php if ($is_text) { ?>
                    <?php if ($isNormalEditor) { ?>
                        <a title="Advanced" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(base64_encode(trim(FM_PATH))) ?>&amp;edit=<?php echo urlencode(base64_encode($file)) ?>&amp;env=ace"><i class="fa fa-pencil-square-o"></i> <?php echo lng('AdvancedEditor') ?></a>
                    ...
                    <?php } else { ?>
                        <a title="Plain Editor" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(base64_encode(trim(FM_PATH))) ?>&amp;edit=<?php echo urlencode(base64_encode($file)) ?>"><i class="fa fa-text-height"></i> <?php echo lng('NormalEditor') ?></a>
        ...
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            if ($parent !== false) {
                ...
                <tr><?php if (!FM_READONLY): ?>
                    ...
                    <td class="border-0"><a href="?p=<?php echo urlencode(base64_encode($parent)) ?>"><i class="fa fa-chevron-circle-left go-back"></i> ..</a></td>
            ...
            $ii = 3399;
            foreach ($folders as $f) {
                $is_link = false; //is_link($path . '/' . $f);
                $img = $is_link ? 'icon-link_folder' : 'fa fa-folder-o';
                $modif_raw = time(); //$ops->filemtime($path . '/' . $f);
                ...
                <tr>
                    ...
                    <td>
                        <div class="filename"><a href="?p=<?php echo urlencode(base64_encode(trim(FM_PATH . '/' . $f, '/'))) ?>"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win(fm_enc($f)) ?>
                    ...
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td><?php if (!FM_READONLY): ?><a title="Change Permissions" href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;chmod=<?php echo urlencode(base64_encode($file)) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?>
                        </td>
                        <td><?php echo $owner['name'] . ':' . $group['name'] ?></td>
                    <?php endif; ?>
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete')?>" href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;del=<?php echo urlencode(base64_encode($file)) ?>" onclick="return confirm('<?php echo lng('Delete').' '.lng('Folder').'?'; ?>\n \n ( <?php echo urlencode(base64_encode($file)) ?> )');"> <i class="fa fa-trash-o" aria-hidden="true"></i></a>
                            <a title="<?php echo lng('Rename')?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                            <a title="<?php echo lng('CopyTo')?>..." href="?p=&amp;copy=<?php echo urlencode(base64_encode(trim(FM_PATH . '/' . $f, '/'))) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink')?>" href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;st=<?php echo urlencode(base64_encode($f)) ?>" target="_blank"><i class="fa fa-link" aria-hidden="true"></i></a>
                    </td>
                </tr>
                <?php
                flush();
            ...
            $ik = 6070;
            foreach ($files as $f) {
                $fileObject = $objects[array_search($f, array_column($objects, 'Name'))];
                $is_link = false; //is_link($path . '/' . $f);
                $img = $is_link ? 'fa fa-file-text-o' : fm_get_file_icon_class($path . '/' . $f);
                $modif_raw = strtotime(explode('.', $fileObject['ModTime'])[0]); //$ops->filemtime($path . '/' . $f);
                $modif = date(FM_DATETIME_FORMAT, $modif_raw);
                $file_url = FM_SELF_URL . "/?p=" . urlencode(base64_encode(FM_PATH)) . "&st=" . urlencode(base64_encode($f));
                $filesize_raw = $fileObject['Size']; //fm_get_size($path . '/' . $f);
                $filesize = fm_get_filesize($filesize_raw);
                ...
                <tr>
                    ...
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td><?php if (!FM_READONLY): ?><a title="<?php echo 'Change Permissions' ?>" href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;chmod=<?php echo urlencode(base64_encode($file)) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?>
                        </td>
...
{
    // clouds don't have file permissions
    return true;
...
{
    global $ops;
    $mime = $ops->mime_content_type($file_path);
    if ($mime !== false) {
        return $mime;
    } else {
        return '--';
...
function fm_get_directorysize($directory) {
    ...
    foreach($ops->scandir($directory, true, true) as $file)
    if (!$file['IsDir'])
        {   $size+=$file['Size'];
            $count++;
        }
    ...
    global $ops;
     if($dir) {
         ...
         foreach ($search as $file) {
             if (!$file['IsDir']) {
                 $fileName = $file['Name'];
                 $location = trim(str_replace(FM_ROOT_PATH, '', $dir), '/');
                 $files[] = array(
                     "name" => $fileName,
                     "type" => "file",
                     "path" => $location,
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            if ($path != '') {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $parent_enc = urlencode(base64_encode($parent));
            ...
            <div class="col-xs-6 col-sm-7 text-right">
                <ul class="navbar-nav mr-auto float-right <?php echo fm_get_theme();  ?>">
                    ...
                    <?php if (!FM_READONLY): ?>
                    <li class="nav-item">
                        <a title="<?php echo lng('Upload') ?>" class="nav-link" href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;upload"><i class="fa fa-cloud-upload" aria-hidden="true"></i> <?php echo lng('Upload') ?></a>
                    </li>
                    <li class="nav-item">
                    ...
                    <li class="nav-item avatar dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right <?php echo fm_get_theme(); ?>" aria-labelledby="navbarDropdownMenuLink-5">
                            <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Settings') ?>" class="dropdown-item nav-link" href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;settings=1"><i class="fa fa-cog" aria-hidden="true"></i> <?php echo lng('Settings') ?></a>
                            <?php endif ?>
                            <a title="<?php echo lng('Help') ?>" class="dropdown-item nav-link" href="?p=<?php echo urlencode(base64_encode(FM_PATH)) ?>&amp;help=2"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> <?php echo lng('Help') ?></a>
...
<script>
    ...
    function newfolder(e) {
        ...
        null !== t && "" !== t && n && (window.location.hash = "#", window.location.search = "p=" + encodeURIComponent(btoa(e)) + "&new=" + encodeURIComponent(btoa(t)) + "&type=" + encodeURIComponent(n))
    }
    function rename(e, t) {var n = prompt("New name", t);null !== n && "" !== n && n != t && (window.location.search = "p=" + encodeURIComponent(btoa(e)) + "&ren=" + encodeURIComponent(btoa(t)) + "&to=" + encodeURIComponent(btoa(n)))}
    ...
    function backup(e, t) { //Create file backup with .bck
        var n = new XMLHttpRequest,
            a = "path=" + btoa(e) + "&file=" + btoa(t) + "&type=backup&ajax=true";
    ...
    function search_template(data) {
        ...
        $.each(data, function (key, val) {
            response += `<li><a href="?p=${btoa(val.path)}&view=${btoa(val.name)}">${val.path}/${val.name}</a></li>`;
        });
        return response;
    }
    //search
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE):
        $ext = "javascript";
        $ext = pathinfo(base64_decode($_GET["edit"]), PATHINFO_EXTENSION);
        ?>
